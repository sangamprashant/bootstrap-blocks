...
function PricingDayB() {
  const { isDarkMode } = useContext(AppContext);
  const experts = [
    {
      title: "Finance Expert",
      description:
        "Experts in financial modeling and valuation startup funding interim CFO work and market sizing.",
    ...
    },
    {
      title: "Developers",
      description:
        "Seasoned software engineers, coders, and architects with expertise across hundreds of technologies with developing tools.",
      onlineEveryDay: "8K online every day",
      ...
      <div className="row g-2 mt-3">
        {experts.map((expert, index) => (
          <div className="col-md-4" key={index}>
            <div className={`card p-3 py-4 shadow ${isDarkMode?"bg-dark text-light":""}`}>
              <div className="text-left">
                <img src="logo.png" width="40" alt={expert.title} />
              </div>
              <h5>{expert.title}</h5>
              ...
              <div className="mt-5">
                <PrimaryButton label={expert.buttonText} />
              </div>
            </div>
          </div>
        ))}
