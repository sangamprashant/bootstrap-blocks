...
const counterData = [
  {
    icon: <Mug height="80" width="50" stroke="black" fill="none" />,
    count: 2147,
    title: "BEER ORDERED",
  },
  {
  ...
  {
    icon: <Code height="80" width="50" stroke="black" fill="none" />,
    count: 289,
      ...
      <div className="row">
        {counterData.map((item, index) => (
          <div className="col-md-3" key={index}>
            <div
              className={`counter-box d-flex flex-column justify-content-center align-items-center bg-white`}
            >
              <i>{item.icon}</i>
              <p>{item.title}</p>
